class AddIndexForAccountTransactionsOnLoanPayments < ActiveRecord::Migration[5.2]
  def change
    ...
    add_index(:account_transactions, [:transacted_at, :subsidiary_id], name: 'index_account_transactions_loan_payments', where: "(transaction_type = 'loan_payment' AND subsidiary_type = 'Loan' AND amount > 0 AND status = 'approved')")
  end
end
