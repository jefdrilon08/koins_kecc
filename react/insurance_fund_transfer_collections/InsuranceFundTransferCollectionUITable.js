...
export default class InsuranceFundTransferCollectionUITable extends React.Component {
  ...
  buildRecords() {
    ...
    for(var i = 0; i < this.props.data.data.records.length; i++) {
      ...
      for(var j = 0; j < this.props.data.data.records[i].records.length; j++) {
        ...
        if(this.props.data.status == "pending") {
          components.push(
            <td key={"insurance-fund-transfer-payment-" + j} className="text-right">
              <strong>
                <a 
                  href="#"
  ...
  buildTotals() {
    ...
    for(var i = 0; i < totals.length; i++) {
      records.push(
        <td key={"total-insurance-fund-transfer-payment-" + i} className="text-right">
          <strong>
  ...
  handleModalConfirm() {
    ...
    $.ajax({
      url: "/api/v1/insurance_fund_transfer_collections/modify_transaction_record",
      method: "POST",
      data: data,
      success: function(response) {
        //window.location.reload();
