...
Modal.setAppElement("#equity-withdrawal-collection-content")

export default class EquityWithdrawalCollectionUITable extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleRemoveRecord(id) {
    ...
    $.ajax({
      url: "/api/v1/equity_withdrawal_collections/remove_member",
      method: 'POST',
      data: {
  ...
  buildRecords() {
    ...
    for(var i = 0; i < this.props.data.data.records.length; i++) {
      ...
      for(var j = 0; j < this.props.data.data.records[i].records.length; j++) {
        ...
        if(this.props.data.status == "pending") {
          components.push(
            <td key={"equity-withdrawal-payment-" + j} className="text-right">
              <strong>
                <a 
                  href="#"
  ...
  buildTotals() {
    ...
    for(var i = 0; i < totals.length; i++) {
      records.push(
        <td key={"total-equity-withdrawal-payment-" + i} className="text-right">
          <strong>
  ...
  handleModalConfirm() {
    ...
    $.ajax({
      url: "/api/v1/equity_withdrawal_collections/modify_transaction_record",
      method: "POST",
      data: data,
      success: function(response) {
        //window.location.reload();
