module Api
  module Members
    class EquitiesAccountsController < ::Api::FrontController
      ...
      def show
        ...
        if id.blank?
          ...
          equity_account = MemberAccount.find_by_id_and_member_id_and_account_type(id, @member.id, "EQUITY")

          if equity_account.blank?
            render json: { errors: ["insurance account not found"] }, status: :unprocessable_entity
          else
            cmd = ::Members::BuildEquityAccount.new(equity_account: equity_account)
