...
export default class TimeDepositCollectionUITable extends React.Component {
  ...
  handleRemoveRecord(id) {
    ...
    $.ajax({
      url: "/api/v1/time_deposit_collections/remove_member",
      method: 'POST',
      data: {
        ...
        components.push(
          <td key={"na-" + member.id + "-" + j} className="text-right">
            ...
            <div className="text-muted">
              <small>
                {paymentRecord.lock_in_period.num_days} Days / {paymentRecord.lock_in_period.num_months} Months / Interest Rate: {paymentRecord.lock_in_period.interest_rate * 100}% / Expected Interest: {paymentRecord.lock_in_period.expected_interest}
              </small>
            </div>
          </td>
        )
  ...
  handleModalConfirm() {
    ...
    $.ajax({
      url: "/api/v1/time_deposit_collections/modify_transaction_record",
      method: "POST",
      data: data,
      success: function(response) {
        //window.location.reload();
