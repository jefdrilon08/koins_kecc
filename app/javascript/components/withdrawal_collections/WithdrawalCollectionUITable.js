...
export default class WithdrawalCollectionUITable extends React.Component {
  ...
  handleRemoveRecord(id) {
    ...
    $.ajax({
      url: "/api/v1/withdrawal_collections/remove_member",
      method: 'POST',
      data: {
  ...
  buildRecords() {
    ...
    for(var i = 0; i < this.props.data.data.records.length; i++) {
      ...
      for(var j = 0; j < this.props.data.data.records[i].records.length; j++) {
        ...
        if(this.props.data.status == "pending") {
          components.push(
            <td key={"withdrawal-payment-" + j} className="text-right">
              <strong>
                <a 
                  href="#"
  ...
  buildTotals() {
    ...
    for(var i = 0; i < totals.length; i++) {
      records.push(
        <td key={"total-withdrawal-payment-" + i} className="text-right">
          <strong>
  ...
  handleModalConfirm() {
    ...
    $.ajax({
      url: "/api/v1/withdrawal_collections/modify_transaction_record",
      method: "POST",
      data: data,
      success: function(response) {
        //window.location.reload();
