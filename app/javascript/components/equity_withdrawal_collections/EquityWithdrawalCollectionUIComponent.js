...
import EquityWithdrawalCollectionUITable from './EquityWithdrawalCollectionUITable';
import AccountingEntryPreview from '../accounting/AccountingEntryPreview';
import AddRecord from './AddRecord';
import {numberWithCommas} from '../utils/helpers';

export default class EquityWithdrawalCollectionUIComponent extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.fetchEquityWithdrawalCollectionData();
  }

  fetchEquityWithdrawalCollectionData() {
    var context = this;

    $.ajax({
      url: "/api/v1/equity_withdrawal_collections/fetch",
      method: 'GET',
      data: {
        id: this.props.id
      },
      ...
      error: function(response) {
        ...
        alert("Error in fetching equity withdrawal_collection");
      }
    });
  }
  ...
  saveParticular() {
    ...
    $.ajax({
      url: "/api/v1/equity_withdrawal_collections/update_particular",
      method: 'POST',
      data: {
    ...
    if(this.state.isLoading) {
      ...
      return (
        <div>
          ...
          />
          <hr/>
          <EquityWithdrawalCollectionUITable
            id={this.props.id}
            data={this.state.data}
