  ...
  class BuildAccountingEntry
    ...
    def build_particular
      ...
      if @claim.blip? 
        ...
        particular = "CLIP claim of #{@claim.member.full_name} for #{@claim_data[:type_of_loan]} : #{@claim_data[:amount]} amount payable to beneficiary , #{@claim_data[:amount_payable_to_creditor]} amount payable to #{@claim_data[:creditors_name]}"
      elsif @claim.hiip?
