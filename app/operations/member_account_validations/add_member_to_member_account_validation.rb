module MemberAccountValidations
	class AddMemberToMemberAccountValidation
		attr_accessor :member_account_validation, :member, :resignation_date, :member_classification

		def initialize(member_account_validation:, member:, resignation_date:, member_classification:)
      @member                        = member
      @resignation_date              = resignation_date
      @member_classification         = member_classification
      # @resignation_date            = member.member_accounts.first.member_account_transactions.last.transaction_date.to_s 
      # COMMENT OUT
      ...
      @data = ::MemberAccountValidations::GenerateMemberAccountDetailsForLifAndRfForValidation.new(member: @member, lif_member_account: @lif_member_account, rf_member_account: @rf_member_account, resignation_date: @resignation_date).execute!
      ...
      if @years >= 1 && @rf_current_balance >= 260 
        @interest = ::MemberAccountValidations::GenerateInterest.new(member_account: @rf_member_account, insured_amount: @rf_insured_amount, num_weeks: @rf_num_weeks, num_weeks_past_due: @rf_num_weeks_past_due).execute!
      ...
      if w >= 1
        @equity_interest = ::MemberAccountValidations::GenerateEquityInterest.new(lif_50_percent: @lif_50_percent, resignation_date: @resignation_date, equity_interest_implementation_date: @equity_interest_implementation_date).execute!
