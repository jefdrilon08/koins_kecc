module Icpr
  class BuildAccountingEntry
    def initialize(config:)
      ...
      @accounting_code_interest_on_share_capital_payable      = AccountingCode.find(Settings.icpr.accounting_code_interest_on_share_capital_payable)
      @accounting_code_due_to_members                         = AccountingCode.find(Settings.icpr.accounting_code_due_to_members)
      @accounting_code_deposits_on_share_capital_subscription = AccountingCode.find(Settings.icpr.accounting_code_deposits_on_share_capital_subscription)
    ...
    def default_particular
      "To record declaration of interest for share capital for branch #{@branch.name} #{@data[:year]}"
    end
  end
end
