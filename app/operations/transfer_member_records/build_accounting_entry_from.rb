module TransferMemberRecords
  class BuildAccountingEntryFrom
    def initialize(config:)
      @config               = config
      @accounting_entry     = @config[:accounting_entry_from]
      @acc_entry_to         = @config[:accounting_entry_to]
    ...
    def build_credit_journal_entries!
      ...
      #   "To Record Transfer Member/s From  #{@accounting_entry[:branch]} to #{@acc_entry_to[:branch]}"
      # end
  end
end
