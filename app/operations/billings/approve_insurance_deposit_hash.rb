module Billings
  class ApproveInsuranceDepositHash
    def initialize(config:)
      @config             = config
      @date_paid          = @config[:date_paid]
      @insurance_deposit  = @config[:insurance_deposit]
      @user               = @config[:user]
      @particular         = @config[:particular]
      @amount             = @insurance_deposit[:amount].try(:to_f).round(2)

      @transaction_type = "deposit"

      @member_account = MemberAccount.find(@insurance_deposit[:member_account_id])
