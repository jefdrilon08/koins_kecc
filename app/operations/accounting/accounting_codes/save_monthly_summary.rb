    ...
    class SaveMonthlySummary
      ...
      def execute!
        entries = JournalEntry.joins(:accounting_entry).select(
                    ...
                    "EXTRACT(YEAR from accounting_entries.date_posted) <= ? AND EXTRACT(MONTH from accounting_entries.date_posted) <= ? AND accounting_entries.status = ? AND journal_entries.accounting_code_id = ? AND accounting_entries.branch_id = ?",
                    @year,
                    @month,
                    "approved",
                    @accounting_code.id,
