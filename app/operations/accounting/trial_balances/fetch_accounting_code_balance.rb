module Accounting
  module TrialBalances
    class FetchAccountingCodeBalance
      def initialize(config:)
        ...
        @data = {
          ...
          accounting_fund: {
            id: @accounting_fund.try(:id),
            name: @accounting_fund.try(:name)
          },
      ...
      def execute!
        ...
        category = @accounting_code.category.downcase

        data[:"#{category}_beginning"] = build_entries(category: category.upcase, phase: :beginning)
      ...
      def filter_entries(category, phase, post_type, accounting_fund_id = nil, is_yearly:)
        entries = ReadOnlyAccountingEntry
          ...
          .where(
            status: "approved",
            branch_id: @branch.id,
            journal_entries: { post_type: post_type, accounting_code_id: @accounting_code.id }
          )
        ...
        when :current
          ...
          entries = entries.where("EXTRACT(YEAR FROM accounting_entries.date_posted) = ?", @start_date.year) if is_yearly
        when :ending
          entries = entries.where("accounting_entries.date_posted <= ?", @end_date)
      ...
      def compute_totals(category, phase, is_yearly:)
        ...
        entries = [@accounting_code].map do |accounting_code|
          debit = 0.00
          credit = 0.00
      ...
      def build_entries(category:, phase:)
        ...
        compute_totals(category, phase, is_yearly: is_yearly)
      end
