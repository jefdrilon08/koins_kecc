module MembershipPaymentCollections
  class ApproveEquityPaymentHash
    def initialize(config:)
      @config         = config
      @date_paid      = @config[:date_paid]
      @equity_payment = @config[:equity_payment]
      @user           = @config[:user]
      @particular     = @config[:particular]
      @amount         = @equity_payment[:amount].try(:to_f).round(2)

      @transaction_type = "deposit"

      @member_account = MemberAccount.find(@equity_payment[:member_account_id])
