module Reports
  class GenerateCollectionsHiipReportExcel
    def initialize(branch:, cluster:, start_date:, end_date:)
      ...
      if @cluster.present?
        @branch       = Branch.where(cluster_id: @cluster)
        @hiip_claim   = HiipClaim.where("date_posted >= ? AND date_posted <= ? AND branch_id IN (?)", @start_date, @end_date, @branch.ids)
      elsif @cluster.present? && @branch.present?
        @branch = branch
        @hiip_claim   = HiipClaim.where("date_posted >= ? AND date_posted <= ? AND branch_id = ?", @start_date, @end_date, @branch)
      end
