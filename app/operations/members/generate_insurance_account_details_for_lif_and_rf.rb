module Members
  class GenerateInsuranceAccountDetailsForLifAndRf < ::Api::FrontController
  ...
  def initialize(config:)
    ...
    @date_of_death              = @config[:date_of_death].to_date
    @account_transactions       = ReadOnlyAccountTransaction.where("subsidiary_id IN (?) AND transacted_at <= ?", [@lif_insurance_account.id, @rf_insurance_account.id], @date_of_death)
  ...
  def build_lif_data!
    ...
    @life_num_days   = (@date_of_death - @start_date).to_i
    ...
    @data[:date_of_death]            = @date_of_death.strftime("%B %d, %Y")
  ...
  def build_rf_data!
    ...
    @rf_num_days   = (@date_of_death - @start_date).to_i
