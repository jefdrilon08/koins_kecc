module Members
  class Reinstate
    def initialize(member:, reinstatement_date:, reinstate_by:, date_stop:)
      ...
      # Initialize reinstatement as an array
      @member_data[:reinstatement] ||= []
    end

    def execute!
      # Ensure reinstatement is treated as an array
      @member_data[:reinstatement] ||= []

      if @member_data[:reinstatement].empty?
        ...
        @member_data[:reinstatement] << {
          ...
          reinstate_by: @reinstate_by,
          date_reinstated: Date.today,
          old_recognition_date: @recognition_date,
          is_reinstated: true
        }
      else
        reinstatement_date = @member_data[:reinstatement][0]["reinstatement_date"]
        @member_data[:reinstatement] << {
          reinstatement_date: @reinstatement_date,
          num_weeks_past_due: @num_weeks_past_due,
          amount_to_paid: @amount_to_paid,
